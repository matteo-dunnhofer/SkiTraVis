...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == "trident_pro_au":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident_au(visible)
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                if self.frame_sample_mode == "trident_pro_au":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident_au(visible)
                elif self.frame_sample_mode in ["trident", "trident_pro"]:
                    template_frame_ids, search_frame_ids = self.get_frame_ids_trident(visible)
                elif self.frame_sample_mode == "stark":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_stark(visible, seq_info_dict["valid"])
                else:
            ...
            try:
                ...
                # negative samples
                    ...
                    if is_video_dataset:
                        ...
                        #print(search_frame_ids)
                        if search_frame_ids is None:
                            search_frames, search_anno, meta_obj_test = self.get_one_search()
                            #search_frames, search_anno, meta_obj_test = self.get_one_search_random_box()
                        else:
    ...
    def sample_seq_from_dataset_invisible(self, dataset, is_video_dataset):

        # Sample a sequence with enough visible frames
        enough_invisible_frames = False
        while not enough_invisible_frames:
            # Sample a sequence
            seq_id = random.randint(0, dataset.get_num_sequences() - 1)
            ...
            enough_invisible_frames = (1 - visible).type(torch.int64).sum().item() > 2

            enough_invisible_frames = enough_invisible_frames or not is_video_dataset
        return seq_id, visible, seq_info_dict
    
    def sample_seq_from_dataset_random_box(self, dataset, is_video_dataset):
        W = 1080
        H = 720
        # Sample a sequence with enough visible frames
        enough_invisible_frames = False
        while not enough_invisible_frames:
            ...
            for bi in range(bbox.shape[0]):
                x = bbox[bi,0] + random.choice([-1.0, 1.0]) * random.uniform(0.1, 5.0) * bbox[bi,2]
                y = bbox[bi,1] + random.choice([-1.0, 1.0]) * random.uniform(0.1, 5.0) * bbox[bi,3]
                w = bbox[bi,2] * random.uniform(0.1, 3.0)
                h = bbox[bi,3] * random.uniform(0.1, 3.0)

                random_box = torch.tensor([x,y,w,h])
                gt_box = torch.tensor(np.copy(bbox[bi]))
                #print(box_iou(box_xywh_to_xyxy(random_box).unsqueeze(0), box_xywh_to_xyxy(gt_box).unsqueeze(0)))
                #print('CAZZO')
                iou, _ = box_iou(box_xywh_to_xyxy(random_box).unsqueeze(0), box_xywh_to_xyxy(gt_box).unsqueeze(0))
                iou = iou[0]
                visible = iou >= 0.7
            ...
            bbox = torch.stack(random_boxes, 0)
            visible = torch.ByteTensor(visibles)
            #print(bbox.shape)

            #print(bbox, visible)

            seq_info_dict['bbox'] = bbox
            seq_info_dict['visible'] = visible
    ...
    def get_one_search(self):
        ...
        is_video_dataset = dataset.is_video_sequence()
        # sample a sequence
        seq_id, visible, seq_info_dict = self.sample_seq_from_dataset_invisible(dataset, is_video_dataset)
        # sample a frame
        if is_video_dataset:
    ...
    def get_one_search_random_box(self):
        # Select a dataset
        dataset = random.choices(self.datasets, self.p_datasets)[0]
        ...
        if random.random() < 0.5:
            ...
            seq_id, visible, seq_info_dict = self.sample_seq_from_dataset_random_box(dataset, is_video_dataset)     
    ...
    def get_frame_ids_trident_au(self, visible):
        # get template and search ids in a 'trident' manner
        template_frame_ids_extra = []
        search_frame_ids = None
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0 or search_frame_ids is None:
            template_frame_ids_extra = []
            # first randomly sample two frames from a video
            template_frame_id1 = self._sample_visible_ids(visible, num_ids=1)  # the initial template id
            search_frame_ids = self._sample_visible_ids(visible, num_ids=1) #, min_id=template_frame_id1[0]+1)  # the search region id
            # get the dynamic template id

            if search_frame_ids is not None:
                for template_frame_id, search_frame_id in zip(template_frame_id1, search_frame_ids):

                    #if template_frame_id >= search_frame_id:
                    #    template_frame_id_extra =  search_frame_id - 1
                    #else:
                    #    template_frame_id_extra = search_frame_id - 1
                    ...
                    if search_frame_id is None:
                        ...
                        template_frame_id_extra = search_frame_id - 1 if search_frame_id > 0 else 0

                        template_frame_ids_extra.append(template_frame_id_extra)

        template_frame_ids = template_frame_id1 + template_frame_ids_extra

        #print(template_frame_ids, search_frame_ids, 'AUUUU')
        return template_frame_ids, search_frame_ids
...
class STARK_ST(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        self.cfg = params.cfg
        self.device = str(getattr(params, 'device', 'cuda'))
    ...
    def track_gt(self, image, gt_conf, info: dict = None):
        H, W, _ = image.shape
        self.frame_id += 1
        # get the t-th search region
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image, self.state, self.params.search_factor,
        ...
        self.state = clip_box(self.map_box_back(pred_box, resize_factor), H, W, margin=10)
        # get confidence score (whether the search region is reliable)
        conf_score = gt_conf #out_dict["pred_logits"].view(-1).sigmoid().item()
        # update template
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and conf_score > 0.5:
                #print(self.frame_id)
    ...
    def update_dynamic_template(self, image, bbox):
        #print(self.frame_id)
        z_patch_arr, _, z_amask_arr = sample_target(image, bbox, self.params.template_factor,
                                                    output_sz=self.params.template_size)  # (x1, y1, w, h)
        template_t = self.preprocessor.process(z_patch_arr, z_amask_arr)
        with torch.no_grad():
            z_dict_t = self.network.forward_backbone(template_t)
    ...
    def redetect(self, image, info: dict = None):
        H, W, _ = image.shape

        h_locations = 3 #int((H / shift_x) - 1)
        shift_y = H / (h_locations+1)

        w_locations = int((W / shift_y) - 1)
        shift_x = shift_y #W / (w_locations+1)

        

        prev_search_factor = copy.copy(self.params.search_factor)

        self.params.search_factor = 2.0
        ...
        for xs in range(w_locations):
            for ys in range(h_locations):

                x, y = xs * shift_x, ys * shift_y
                h = shift_y / self.params.search_factor 

                self.state = [int(x), int(y), int(h), int(h)]

                #print(f'Processing patch {self.state}')
                ...
                pred_box = (pred_boxes.mean(dim=0) * self.params.search_size / resize_factor).tolist()  # (cx, cy, w, h) [0,1]
                # get the final box result
                pred_box = clip_box(self.map_box_back(pred_box, resize_factor), H, W, margin=10)
                # get confidence score (whether the search region is reliable)
                conf_score = out_dict["pred_logits"].view(-1).sigmoid().item()

                redet_boxes.append(pred_box)
                redet_scores.append(conf_score)

        redet_boxes = np.array(redet_boxes)#.astype(np.float32)
        redet_scores = np.array(redet_scores)
        max_idx = np.argmax(redet_scores)

        self.params.search_factor = copy.copy(prev_search_factor)

        #print(redet_boxes[max_idx])

        return redet_boxes[max_idx], redet_scores[max_idx]
